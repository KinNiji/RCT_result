...
st.markdown('将原始数据分为4组（真刺激治疗前、真刺激治疗后、伪刺激治疗前、伪刺激治疗后），之后从所有量表相关的特征中挑出n（n大于等于1）个特征，这n个特征的值按样本相加，分别对真刺激组治疗前后差异、伪刺激组治疗前后差异、真伪刺激组治疗前差异、真伪刺激组治疗后差异进行t检验，最后挑选出:red[真伪刺激组在治疗前无明显差异]，:red[真刺激组治疗前后有明显差异]，且:red[治疗前后真刺激组差异大于假刺激组差异]的特征组合。')
st.markdown('之后将所有满足条件的特征组合，按治疗前后差异最大的进行排列，和按真刺激组差异大于伪刺激组差异进行排列，分别取top k拼接在一起并去重。下面即为筛选出的特征组合。')
...
if index < 0 or index >= len(df):
    ...
    st.markdown('对top 2k的每一个特征组合，其中的每个特征与所有影像与声音特征计算皮尔逊相关系数，取出相关度最高的n个影像和声音特征（n=特征组合中特征个数）。')
